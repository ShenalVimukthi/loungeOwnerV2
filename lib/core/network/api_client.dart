...
class ApiClient {
  ...
  AuthTokensModel? _cachedTokens; // Cache tokens in memory to avoid storage read timing issues
    ...
    _dio.interceptors.add(InterceptorsWrapper(
      onRequest: (options, handler) async {
        ...
        if (tokens != null && !tokens.isExpired) {
          ...
          print('🔐 Token available: true (expires in ${tokens.remainingSeconds}s)');
  ...
  Future<bool> _refreshToken() async {
    try {
      ...
      print('🔄 Sending refresh request with token (first 20 chars): ${tokens.refreshToken.substring(0, 20)}...');
